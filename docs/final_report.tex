...
\icmlcorrespondingauthor{Zhengyao Zhao}{TODO@example.com}
...
Qualitative examples are most useful when they make a numerical difference visible. We would therefore focus on a small set: one example where \expid{E3} preserves scene or character state better than \expid{E4}, one where \expid{E3} and \expid{E5} are similarly fluent but \expid{E5} adds no coherence benefit, and, if space allows, one example where structured prompting is visibly better than plain concatenation.
...
The retrieval run is weaker than \expid{E3} on both likelihood-based and overlap-based metrics. We leave it in the appendix and keep the mainline interpretation unchanged.
