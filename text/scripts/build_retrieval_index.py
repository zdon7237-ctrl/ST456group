"""Save a small retrieval index for the processed data."""
...
def build_retrieval_index(dataset_path: Path, output_path: Path) -> None:
    if not dataset_path.exists():
        raise FileNotFoundError(
            f"Retrieval dataset not found at {dataset_path}. Build the processed data before indexing."
        )

    candidate_records = load_candidate_records(dataset_path)
    if not candidate_records:
        raise ValueError(
            f"No retrieval candidates found in {dataset_path}. The processed data has no usable rows."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build and save a TF-IDF index.")
