"""Build paragraph continuation splits from the raw Holmes texts."""
...
def build_dataset(raw_dir: Path, output_dir: Path, context_size: int, min_chars: int) -> dict[str, list[dict]]:
    ...
    for source in SHERLOCK_SOURCES:
        ...
        # Split within each book so validation and test still come from the same source mix.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build processed paragraph continuation data.")
