"""Check token budgets for one experiment config."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Inspect token statistics for one experiment config.")
    ...
    if config.get("use_retrieval", False):
        # Attach retrieval here so the budget matches the training prompt.
