"""Compare two E5 auxiliary-weight runs by validation loss."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare two E5 auxiliary-weight runs using validation_main_loss."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="If the validation-loss gap is this small, keep both runs in the notes.",
    )
    ...
    # Use validation_main_loss only; the test set should not choose the model.
    if gap_percent <= args.threshold_percent:
        recommendation = (
            f"keep both {left_label} and {right_label} in the notes "
