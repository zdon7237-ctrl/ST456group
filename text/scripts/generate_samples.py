"""Generate continuations from a fine-tuned checkpoint."""
...
def generate_rows(
    ...
    # Use the prompt settings saved during training unless the CLI overrides them.
    ...
    for row in row_iter:
        ...
        # Add the target marker so decoding begins at the continuation boundary.
        ...
        # Leave room for new tokens by trimming the prompt side first.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate continuations from a trained model.")
    ...
    parser.add_argument("--do-sample", action="store_true", default=True,
                        help="Use sampling; pass --no-do-sample for greedy decoding.")
