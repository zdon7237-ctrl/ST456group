"""Training code for the continuation experiments."""
...
def _resolve_path(value: str, config_path: Path) -> str:
    ...
    # Try paths next to the config first; otherwise treat them as project-root paths.
...
def choose_max_target_tokens(
    ...
    # Use a rounded high percentile so one unusually long paragraph does not set the cap.
...
def summarise_token_budget(
    ...
    *,
    ...
    for record in records:
        ...
        # Match the serialized input that the collator will later build.
...
def attach_retrieval(
    ...
    for index, record in enumerate(records):
        query = "\n".join(record["context"])
        # Only retrieve passages that would have been available before this target.
...
def select_negative_candidates(records: list[dict], index: int, num_negative_candidates: int, neg_index: dict | None = None) -> list[str]:
    ...
    if neg_index is not None:
        # Try same-book negatives first, then fall back to the same split.
...
def prepare_training_records(
    ...
    prepared = []
    for index, record in enumerate(records):
        # Pick the context window once so training, generation, and eval use the same view.
        context_window = select_context_window(record["context"], context_size)
...
def save_training_metadata(output_dir: str, config: dict, metadata: dict[str, Any]) -> None:
    ...
    # Save the resolved config with the checkpoint; it makes Colab runs easier to audit.
...
def _build_hf_trainer(config: dict, prepared_train: list[dict], prepared_eval: list[dict], model, tokenizer):
    ...
    # Handle both Transformers argument names; 4.46 renamed evaluation_strategy to eval_strategy.
    ...
    # Transformers 4.46 also renamed tokenizer to processing_class.
    _tokenizer_kwarg = (
        ...
        eval_rows.append(
            {
                ...
                # The evaluator expects generated_text. For validation PPL, the gold target is enough.
...
def evaluate_validation_main_metrics(trainer, tokenizer) -> dict[str, float]:
    ...
    # Score validation PPL on the masked main objective, not on the auxiliary loss.
    return {
