"""Small retrieval helpers for continuation prompts."""
...
def fit_tfidf_retriever(candidates: list[str]) -> dict[str, object]:
    """Fit TF-IDF over passages that are already allowed for retrieval."""
    if not candidates:
...
def query_tfidf_index(
    ...
    """Look up passages in a TF-IDF index.

    The caller should pass only earlier passages for the current target.
    """
    if top_k <= 0:
        return []
