"""Batching and scoring utilities for continuation training."""
...
def build_prompt_with_budget(
    ...
    *,
    ...
    # Drop the oldest context first; the recent paragraphs matter most.
    ...
    # Remove retrieved snippets only after the main context has been squeezed.
    ...
    # If it still does not fit, keep the tail of the prompt.
...
class ContinuationDataCollator:
    ...
    def _encode_feature(self, feature: dict) -> tuple[list[int], list[int], str, list[int]]:
        ...
        # Mask the prompt and target marker; the main loss should only see target text.
    ...
    def __call__(self, features: list[dict]):
        ...
        if candidate_input_ids_batch:
            ...
            for candidate_inputs, candidate_masks, candidate_labels, class_labels in zip(
                ...
                # Pad empty candidate slots so the tensor can reshape to [B, C, T].
...
def compute_candidate_scores(logits, labels):
    ...
    # Normalize by supervised token count so long candidates are not punished unfairly.
...
def encode_target_with_prefix(
    ...
    *,
    ...
    # Recheck the prefix boundary so training and evaluation split targets the same way.
