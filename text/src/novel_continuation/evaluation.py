"""Metric code for the continuation runs."""
...
def build_conditional_ppl_example(
    ...
    *,
    ...
    # Keep the target intact; trim the prompt first when the sequence is too long.
...
def _accumulate_shifted_nll(logits, labels) -> tuple[float, int]:
    ...
    # Count supervised tokens after the causal shift, matching the loss calculation.
...
def compute_perplexity(model, tokenizer, rows: list[dict[str, str]]) -> float:
    ...
    try:
        ...
        row_iter = tqdm(rows, desc="scoring perplexity", unit="row")
    except ImportError:
        row_iter = rows
    ...
    for row in row_iter:
        ...
        # Accumulate token-level NLL so long and short samples are weighted correctly.
...
def summarise_seed_metrics(metrics_by_seed: list[dict[str, float]]) -> dict[str, float]:
    ...
    for key in metric_keys:
        ...
        # Seed variance only makes sense for sampled generation metrics.
